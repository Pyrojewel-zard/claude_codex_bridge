...
def _copy_claude_auth_source(source: Path, target: Path) -> bool:
    """Project a present auth file via symlink (preferred) with copy fallback.

    Distinguishes absence from read failure: a missing source returns False
    (target preserved — e.g. managed login stays when the source logs out),
    while a present-but-unreadable source raises RuntimeError.
    """
    ...
    try:
        _sync_file_as_symlink(source_path, target)
    except OSError as exc:
        raise RuntimeError(
