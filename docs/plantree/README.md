...
| [codex-reconnect](plans/codex-reconnect/README.md) | Implemented; qualification open | Arm one `codex-reconnect`-managed Codex CLI with `$reconnect on/off` for same-model recovery after terminal network or service-overload failures, without tmux or CCB runtime authority. |
...
| [provider-auth-authority](plans/provider-auth-authority/README.md) | Planning | Define one-way external Provider auth/config inheritance, CCB-local explicit API authority, and server-side-safe credential ownership without reverse interference. |
