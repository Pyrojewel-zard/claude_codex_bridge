...
\begin{lstlisting}
ask cancel clear compact cleanup kill ps ping watch pend queue trace
...
\cmd{compact} 是项目上下文控制命令，不是 mailbox 任务。它通过
\src{project_compact_context} RPC，对指定 agent（省略时为全部 configured
agents）向 tmux pane 发送 provider-native compaction 命令。发送前复用
\cmd{clear} 的 busy/queue gate；provider 没有已验证命令时必须 fail closed，
返回 \src{unsupported}，不得把未知控制词当作普通 prompt 发送。该命令不清理
\src{.ccb}、workspace、auth、session、日志或 memory。
