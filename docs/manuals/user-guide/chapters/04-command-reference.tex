...
\cmd{ccb compact [agent...]} & 向 provider pane 发送已验证的 native context-compaction 命令（例如 Codex/Claude 的 \cmd{/compact}、Gemini/Qwen/Droid 的 \cmd{/compress}、Crush 的 \cmd{/summarize}）；busy/queued agent 会被阻止，未验证 provider 返回 \src{unsupported}；不删除任何项目状态。 \\
